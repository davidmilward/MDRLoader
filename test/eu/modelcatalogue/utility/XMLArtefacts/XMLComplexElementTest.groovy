package eu.modelcatalogue.utility.XMLArtefacts

/**
 * Created by davidmilward on 03/03/2014.
 */
class XMLComplexElementTest extends GroovyTestCase {
    XMLComplexElement comElement

    void setUp() {
        super.setUp()
        comElement = new XMLComplexElement()
        comElement.addSchemaAttribute("","")

    }

    void testAddSchemaAttribute() {


    }

    void testGetSchemaAttribute() {

    }

    void testAddEnumItem() {

    }

    void testAddElement() {

    }

    void testAddRestriction() {

    }

    void testAddElementAttribute() {

    }

    void testGetRestrictionBase() {

    }

    void testSetRestrictionBase() {

    }

    void testGetbAbstract() {

    }

    void testIsbAbstract() {

    }

    void testSetbAbstract() {

    }

    void testGetbMixed() {

    }

    void testIsbMixed() {

    }

    void testSetbMixed() {

    }

    void testGetbFixed() {

    }

    void testIsbFixed() {

    }

    void testSetbFixed() {

    }

    void testGetbDefault() {

    }

    void testIsbDefault() {

    }

    void testSetbDefault() {

    }

    void testGetSchemaAttributeMap() {

    }

    void testSetSchemaAttributeMap() {

    }

    void testGetElementList() {

    }

    void testSetElementList() {

    }

    void testGetRestrictionList() {

    }

    void testSetRestrictionList() {

    }

    void testGetElementAttributeList() {

    }

    void testSetElementAttributeList() {

    }

    void testGetTypeEnumerations() {

    }

    void testSetTypeEnumerations() {

    }

    void testGetReference() {

    }

    void testSetReference() {

    }

    void testGetContainedElements() {

    }

    void testSetContainedElements() {

    }
}
